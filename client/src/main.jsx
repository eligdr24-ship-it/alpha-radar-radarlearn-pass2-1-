...
function OpportunityTable({items, onSelect}) {
  return <div className="tableWrap"><table><thead><tr><th>#</th><th>Coin</th><th>Direction</th><th>Conviction</th><th>Confidence</th><th>Risk</th><th>Freshness</th><th>Consensus</th><th>Zone</th><th>Target 1</th><th>Target 2</th><th>Stretch</th></tr></thead><tbody>{items.slice(0,8).map((op,i)=><tr key={op.symbol} onClick={()=>onSelect(op)}><td>{i+1}</td><td><div className="coinCell"><span className="coinAvatar small">{op.symbol[0]}</span><div><b>{op.symbol}</b><small>{op.name}</small>{op.dataSource&&<small className={"srcBadge "+(/LIVE/.test(op.dataSource)?"live":/STALE/.test(op.dataSource)?"stale":"mock")}>{op.dataSource}</small>}</div></div></td><td><Pill tone={op.direction==='LONG'?'long':'short'}>{op.direction}</Pill></td><td><ScoreRing score={op.conviction} danger={op.direction==='SHORT'} /></td><td>{op.confidence}%</td><td><span className={op.risk==='High'?'red':op.risk==='Low'?'green':'yellow'}>{op.risk}</span></td><td>{op.signals.freshness}</td><td>{op.consensus}%</td><td>{op.display.buyZone}</td><td>{op.display.target1}</td><td>{op.display.target2}</td><td>{op.display.stretch}</td></tr>)}</tbody></table></div>
}
